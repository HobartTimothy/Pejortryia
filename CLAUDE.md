...
bot/
    __init__.py      → main() — aiohttp webhook server + /health /hook endpoints
    factory.py       → create_bot() + create_dispatcher()
config.py            → pydantic-settings (BOT_TOKEN, WEBHOOK_*, LOG_LEVEL, ADMIN_IDS)
...
Required: `BOT_TOKEN`, `WEBHOOK_BASE_URL` (HTTPS prefix, no trailing slash). Optional: `WEBHOOK_PATH`, `WEBHOOK_SECRET`, `WEBHOOK_HOST`, `WEBHOOK_PORT`, `LOG_LEVEL`, `ADMIN_IDS`. See `.env`.
